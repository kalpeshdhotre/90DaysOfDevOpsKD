...
Password input is hidden while typing — nothing appears on screen, which is normal behaviour.
...
**Why `chown root:developers` instead of `chgrp developers`:**

`chown user:group` sets both the owner and group in one command. Using `chgrp` alone only changes the group — it leaves the owner untouched.

During practice, the directory was initially created with a stray UID `775` as the owner (not a named user). When `chgrp` ran, it only fixed the group column and left `775` as the owner. Linux displayed the raw number `775` because no user with that UID exists in `/etc/passwd` — it can only show a name when it finds a match.

`chmod 775` had nothing to do with this — `chmod` only sets permission bits and never touches the owner column. The `775` in the owner column came from the UID at directory creation time, not from `chmod`.

**Fix applied:**

```bash
sudo chown root /opt/dev-project
```

After this, `ls -alh` showed `root` as the owner correctly.
...
| `chown root:group /path`      | Set both owner and group of a directory |
| `chown root /path`            | Fix owner if it shows as a UID number   |
