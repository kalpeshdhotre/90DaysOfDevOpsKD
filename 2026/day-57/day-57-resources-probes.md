...
- **Requests** are what the scheduler uses to decide *where* a Pod can fit. Every node needs enough allocatable CPU/memory left to cover the sum of requests of the Pods already placed on it plus the new one.
- **Limits** are what the kubelet enforces at *runtime* once the Pod is already running, capping how much CPU/memory the container is allowed to actually use.
...
![Liveness probe restarts](screenshots/task4-liveness-restarts-1.png)
![Liveness probe restarts continued, CrashLoopBackOff](screenshots/task4-liveness-restarts-2.png)
...
![Startup probe watch](screenshots/task6-startup-watch.png)
![Startup probe describe output](screenshots/task6-startup-describe.png)
